...
define ("DB_PASS","onegai123"); // set database password
define ("DB_NAME","tes"); // set database name

$link = mysql_connect(DB_HOST, DB_USER, DB_PASS) or die("Couldn't make connection.");
$db = mysql_select_db(DB_NAME, $link) or die("Couldn't select database");

$databasetable = "thingy";
...
$query = "SELECT name FROM thingy WHERE name = '".$userName."' and email = '".$userMobile."'";
$sql = mysql_query($query);
$recResult = mysql_fetch_array($sql);
$existName = $recResult["name"];
if($existName=="") {
$insertTable= mysql_query("insert into thingy (name, email) values('".$userName."', '".$userMobile."');");
